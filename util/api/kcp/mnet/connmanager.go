...
type ConnManager struct {
	connections map[uint32]iface.IKConnection //管理的链接信息
...
func NewConnManager() *ConnManager {
	return &ConnManager{
		connections: make(map[uint32]iface.IKConnection),
	}
}

// 添加链接
func (cmg *ConnManager) Add(conn iface.IKConnection) {
	// 保护共享资源map，加写锁
	cmg.connLock.Lock()
...
// 删除链接
func (cmg *ConnManager) Remove(conn iface.IKConnection) {
	cmg.connLock.Lock()
	defer cmg.connLock.Unlock()
	// 删除链接信息
...
// 通过ConnID获取链接
func (cmg *ConnManager) Get(connid uint32) (iface.IKConnection,error){
	// 保护共享资源map，加读锁
	cmg.connLock.RLock()
